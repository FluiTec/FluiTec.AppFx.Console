...
internal class Program : DependencyInjectionProgram
{
    ...
    {
        ConsoleHost.ConfigureModule(services, _ => new TestModule());
        return services;
    }
}
...
public class TestModule : ModuleConsoleItem
{
    ...
    public override Command ConfigureCommand()
    {
        ...
        updateCmd.AddOption(new Option<string>("--value", "Value of the the configuration-entry.")
            {IsRequired = false});
        updateCmd.Handler = CommandHandler.Create(new Func<string, string, int>(ProcessEdit));

        cmd.AddCommand(updateCmd);
        return cmd;
    }

    private static int ProcessEdit(string key, string val)
    {
